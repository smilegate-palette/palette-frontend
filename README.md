...
- **관리자 페이지 Figma 반영(`/admin`, `AdminPage.tsx`, node 92:2 데스크탑 / 99:540 모바일 / 99:386 풀스크린)**: 통계 카드(승인대기/총 프로젝트/오늘 댓글/스토리) + 승인 대기 프로젝트 테이블(승인/반려/수정) + 최근 댓글 + HOME 큐레이션 순서 섹션까지 시안대로 구조를 맞췄습니다. 로그인 상태면 헤더에 "관리자" 링크가 보임
  - **로그인 응답 role(USER/ADMIN)로 접근 제어 완료** - role이 ADMIN인 계정에만 "관리자" 링크가 보이고, AdminPage 자체도 role 기준으로 접근을 막습니다
  - 반려 사유는 기획 문서의 "부적절한 컨텐츠/정보 부족/기타" 선택지가 아직 확정 전이라 우선 자유 입력으로 구현
  - **Figma엔 있지만 매칭되는 API가 없어서 못 만든 부분들**: "최근 댓글" 전체 목록(프로젝트를 가로지르는 댓글 조회 API 없음, 프로젝트별 조회만 가능), 댓글 "비공개" 토글(숨김 처리 API 없음), "스토리 업로드" 탭(STORY 관리 API 없음), 승인 대기 목록의 "전체 선택" 일괄 처리(일괄 승인/반려 API 없음) - 전부 화면에 준비중 안내나 미표시로 처리해뒀어요
  - 프로젝트 수정 폼의 "썸네일 이미지 업로드"·"추가 필드"는 저장할 백엔드 필드/엔드포인트가 없어서 화면만 있고 실제로는 저장 안 됨 (프로젝트명/유형/설명만 실제 저장)
  - HOME 큐레이션 순서는 Figma엔 드래그 앤 드롭인데, 별도 라이브러리 없이 ▲▼ 버튼으로 단순화해서 구현
  - 큐레이션 조회(`GET /api/admin/{user_id}/curation`) 응답 스키마가 제네릭 object라 필드명을 추정해서 매핑함 - 실제 데이터로 확인 전까지는 화면에 아무것도 안 뜰 수 있어요
